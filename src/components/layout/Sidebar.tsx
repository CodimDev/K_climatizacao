...
type Props = { open: boolean; onClose: () => void; collapsed: boolean; onToggleCollapse: () => void }
...
export default function Sidebar({ open, onClose, collapsed, onToggleCollapse }: Props) {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-64 ${collapsed ? 'lg:w-16' : 'lg:w-64'} transform transition-transform duration-300 ease-out bg-background border-r border-border ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}> 
        <div className="p-4 border-b border-border cursor-pointer" onClick={onToggleCollapse}>
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-2'}`}>
            <div className="h-6 w-6 rounded-md bg-gradient-to-br from-cyan-500 to-cyan-400 shadow-md" />
            {!collapsed && <div className="font-semibold">K-Climatização</div>}
          </div>
        </div>
        <nav className="p-2 space-y-6 overflow-y-auto h-[calc(100vh-64px)]">
          <div className="space-y-1">
            {menuItems.map(it => {
              ...
              return (
                <Link key={it.to} to={it.to} className={`group relative flex items-center gap-2 rounded-md px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-muted ${active ? 'text-primary bg-muted border-l-2 border-l-primary' : ''}`}> 
                  <Icon className="w-4 h-4" />
                  {collapsed ? (
                    <>
                      <div className="pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-2 whitespace-nowrap rounded-md bg-background border border-border px-2 py-1 text-xs shadow opacity-0 group-hover:opacity-100">{it.label}</div>
                    </>
                  ) : (
